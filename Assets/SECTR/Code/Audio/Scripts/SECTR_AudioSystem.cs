...
public class SECTR_AudioSystem : MonoBehaviour 
{
				...
				if(HDR)
				{
					...
					if(!Local && listenerDistance > srcCue.MinDistance)					
					{
						...
						case SECTR_AudioCue.FalloffTypes.Logrithmic:
							...
							{
								falloff *= (1f - Mathf.Clamp01((listenerDistance - maxDistance) / System.CullingBuffer));
							}
							break;
						}
					}
		...
		if(ShowAudioHUD && (!Application.isEditor || Application.isPlaying))
		{
			...
			if(activeHDRSounds && HUDLineMaterial)
			{
				...
				if(!lineRenderer)
				{
					...
					lineRenderer.numPositions = maxHDRVolumes;
	...
	private static void _UpdateBusPitchVolume(SECTR_AudioBus bus, float effectiveVolume, float effectivePitch)
	{
		if(bus)
		{
			...
			if(Application.isEditor && !Application.isPlaying && !system.Debugging && SceneView.lastActiveSceneView && !SceneView.lastActiveSceneView.audioPlay)
			{
				effectiveVolume = 0f;
			}
